...
public class ShowAppointment extends JFrame{
	...
	public ShowAppointment(PIMCollection pimCollection,String msg,SQLHelper sqlHelper){
		super();
		setName(msg);
	...
	class MyTableModel extends AbstractTableModel{
		...
		public void setValueAt(Object arg0, int row, int col) {
			...
			if(oldValue.equals(newValue))
				return;
			String sql = "delete from appointment where owner = '"+(String)(text[row][0]) +"' and isPublic = '"+(String)(text[row][1])
					+"' and priority = '"+(String)(text[row][2])  +"' and data = '"+(String)(text[row][3])+"' and text = '"+(String)(text[row][4])+"';";
			System.out.println(sql);
			sqlHelper.update(sql);
			text[row][col] = newValue;
			sql = "insert into appointment values('"+(String)(text[row][0]) +"','"+(String)(text[row][1])
