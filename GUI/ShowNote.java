...
public class ShowNote extends JFrame{
	...
	public ShowNote(PIMCollection pimCollection,String msg,SQLHelper sqlHelper){
		super();
		setName(msg);
	...
	class MyTableModel extends AbstractTableModel{
		String[] columnNames = {"owner","isPublic","priority","description"};
		private static final long serialVersionUID = 1L;
		...
		public void setValueAt(Object arg0, int row, int col) {
			...
			if(oldValue.equals(newValue))
				return;
			String sql = "delete from note where owner = '"+(String)(text[row][0]) +"' and isPublic = '"+(String)(text[row][1])
					+"' and priority = '"+(String)(text[row][2]) +"' and text = '"+(String)(text[row][3])+"';";
			System.out.println(sql);
			sqlHelper.update(sql);
			text[row][col] = newValue;
			sql = "insert into note values('"+(String)(text[row][0]) +"','"+(String)(text[row][1])
					+"' ,'"+(String)(text[row][2])  +"' , '"+(String)(text[row][3])+"');";
			System.out.println(sql);
			sqlHelper.update(sql);
