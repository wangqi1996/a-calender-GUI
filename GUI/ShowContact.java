...
public class ShowContact extends JFrame{
	...
	public ShowContact(PIMCollection pimCollection,String msg,SQLHelper sqlHelper){
		super();
		setName(msg);
	...
	class MyTableModel extends AbstractTableModel{
		String[] columnNames = {"owner","isPublic","priority","firstname","lastname","address"};
		private static final long serialVersionUID = 1L;
		...
		public void setDays() {
			...
			for(int i = 0; i < pimCollection.size(); i++) {
				...
				text[i][3] = todo.getFirstName();
				text[i][4] = todo.getLastName();
				text[i][5] = todo.getAddress();
		...
		public void setValueAt(Object arg0, int row, int col) {
			...
			if(oldValue.equals(newValue))
				return;
			String sql = "delete from contact where owner = '"+(String)(text[row][0]) +"' and isPublic = '"+(String)(text[row][1])
					+"' and priority = '"+(String)(text[row][2])  +"' and firstname = '"+(String)(text[row][3])
					+"' and lastname = '"+(String)(text[row][4]) +"' and address = '"+(String)(text[row][5])+"';";
			System.out.println(sql);
			sqlHelper.update(sql);
			text[row][col] = newValue;
			sql = "insert into contact values('"+(String)(text[row][0]) +"','"+(String)(text[row][1])
					+"' ,'"+(String)(text[row][2])  +"' , '"+(String)(text[row][3])+"','"+(String)(text[row][4])+"','"+(String)(text[row][5])+"');";
			System.out.println(sql);
			sqlHelper.update(sql);
